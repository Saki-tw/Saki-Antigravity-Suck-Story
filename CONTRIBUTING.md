...
我們收集**真實的經歷**——不限於 Antigravity，也歡迎所有 Agent、AI、LLM、甚至任何自動化系統的鬼故事。希望你用帶有**都市傳說**或**賽博恐怖**風格的筆觸來記錄。我們在尋找那些機器表現得彷彿帶有**私人恩怨**的時刻。

### 收錄範圍
| 來源 | 描述 |
|------|------|
| **Antigravity** | Claude Agent IDE 的各種災難（本專案原始焦點） |
| **AI Agent 框架** | LangChain, AutoGPT, CrewAI 等框架的崩潰 |
| **LLM 模型** | GPT, Claude, Gemini, LLaMA 等模型的幻覺與背叛 |
| **自動化系統** | 任何以人類無法預料的方式搞砸事情的系統 |
...
| **失語 (The Aphasia)** 🤖 | AI 的幻覺、胡言亂語、或突然忘記一切的時刻。 | Hallucination, Context Lost, Token Overflow |
...
## 異象初現 (The Anomaly)
第一個徵兆是什麼？閃爍的 Log？奇怪的延遲？數字對不上？AI 開始胡言亂語？

## 墜入深淵 (The Descent)
詳細描述 Debug 的過程。發現標準工具完全失效時的絕望。感覺被代碼注視著。

## 恐怖降臨 (The Horror)
根本原因是什麼？是 Race Condition？是 Hallucination？還是...更邪門的東西？
...
*記錄時間：YYYY-MM-DD*
*恐怖程度：💀💀💀💀💀 (1-6 顆骷髏)*
*標籤：殭屍/幻影/水蛭/海妖/迷宮/詛咒/失語*
*來源：Antigravity/Agent/AI/LLM/自動化系統*
```
...
我們鼓勵用詩歌、文學、甚至戲劇的形式來記錄你的遭遇。技術 Log 是證據，但**文學是靈魂的遺書**。
